...
def test_tensor_from_another_tensor():
    t1 = pyad.Tensor(5, d=pyad.MultivariateDerivative({'a': 2, 'b': 3}))
    t2 = pyad.Tensor(t1)
...
def test_radd():
    ...
    """
    t1 = pyad.Tensor(3, d=pyad.MultivariateDerivative({'x': 5, 'y': 10}))
    res = 5 + t1

    assert isinstance(res, pyad.Tensor)
    assert res.value == 8
...
def test_scalar_var_setitem():
    ...
    t = pyad.Tensor([1, 2, 3, 4]) * v1 + v2 - 2 * v3
...
def test_shape_and_len():
    ...
    v3 = pyad.Tensor([0, 1, 2, 3])
    assert v3.shape == (4,)
    assert len(v3) == 4

    v4 = pyad.Tensor([[1, 2, 3], [4, 5, 6]])
    assert v4.shape == (2, 3)
    assert len(v4) == 2
...
def test_stack():
    ...
    t1 = pyad.Tensor([1, 2, 3]) * pyad.var('x', 2)
    t2 = pyad.Tensor([4, 5, 6]) * pyad.var('y', 3)
